...
public class ProductoServiceImpl implements ProductoService {
	...
	public List<ProductoDto> obtenerProductos() {
		// Ordeno los productos por nombre de productos
		List<Producto> productos = productoRepository.findAll();
		productos.sort((a, b) -> a.getNombre().compareTo(b.getNombre()));
		return productoConverter.toContratoList(productos);
	}
	...
	public ProductoDto actualizarProducto(ProductoDto dto) throws Exception {
		...
		if (dto.getId() == null)
			throw new Exception(
					"Nos debe pasar el identificador del producto a modificar");
		// Valido que exista en la base de datos
		Producto oldProducto = productoRepository.findById(dto.getId())
				.orElse(null);
		if (oldProducto == null)
			throw new Exception(
					"El producto que se quiere modificar no existe en la base de datos");
		// Convierto
		Producto producto = productoConverter.toEntidad(dto);
		// Persisto
		producto = productoRepository.save(producto);
	...
	public List<ProductoStockDto> obtenerStockProductos() {
		...
		stocks.sort((a, b) -> a.getProducto().getNombre()
				.compareTo(b.getProducto().getNombre()));
		return productoStockConverter.toContratoList(stocks);
	}

	@Override
	public ProductoStockDto actualizarStockProducto(ProductoStockDto dto)
			throws Exception {
		// Valido dto
		if (dto == null)
			throw new Exception(
					"Nos debe pasar el stock que se quiere actualizar.");
		if (dto.getId() == null)
			throw new Exception(
					"Nos debe pasar el identificador del stock a actualizar");
		if (dto.getCantidad() == null)
			throw new Exception(
					"Nos debe pasar la cantidad de producto a la que se quiere actualizar");
		// Valido que exista
		StockProducto oldStock = stockProductoRepository.findById(dto.getId())
				.orElse(null);
		if (oldStock == null)
			throw new Exception(
					"El stock que quiere actualizar no existe en la base de datos");
	...
	public List<TipoProductoDto> obtenerTipoProductos() {
		return tipoProductoConverter
				.toDtoList(tipoProductoRepository.findAll());
	}
